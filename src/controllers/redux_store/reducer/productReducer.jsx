...
function counterReducer(state = initialState, action) {
  const { UPDATE_PRODUCT_COUNTER, SET_PRODUCTS, SET_CHECKOUT, REMOVE_PRODUCT } =
    ActionTypes;
  switch (action.type) {
    ...
    case REMOVE_PRODUCT: // Handle the remove action
      const filteredCart = state.productCart.filter(
        (item) => item.id !== action.payload.id
      );
      localStorage.setItem("productCart", JSON.stringify(filteredCart));
      return { ...state, productCart: filteredCart };
    default:
      return state;
  }
}
